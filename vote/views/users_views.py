     ...
     if request.method== "POST":
            ...
            if form.is_valid():
                ...
                if code == userDate[2]:
                    ...
                    user=User.objects.create_user(userDate[3]+" "+userDate[4] +str(LastInsertId+1), userDate[0], userDate[1])
                    user.first_name=userDate[3] 
                    user.last_name=userDate[4] 
                    user.save()
                    user=authenticate(username=userDate[3]+" "+userDate[4] +str(LastInsertId+1),password=userDate[1])
                    if user is not None:
                        print(user.username)
                        userDate.clear()
                        login(request, user)  
            ...
            context={
                        'form': codeForm,
                        'error': 'introduce the confirmation code again'
                    }
     return render(request,'vote/AuthenticationCode.html',context)
